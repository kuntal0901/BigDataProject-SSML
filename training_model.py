...
def preprocessing(df):
   ...
   df=df.drop("Descript","Resolution","Dates")
   df_X_train=df      
   rdd=df_X_train.rdd.map(lambda x: (dayofweekmappings[x["DayOfWeek"]],pddistrictmappings[x["PdDistrict"]],addressmappings[x["Address"]],x["X"],x["Y"],x["year"],x["month"],x["date"],x["hour"],x["minute"],categorymappings[x["Category"]]))
